...
if __name__ == '__main__':
    ...
    for i in range(0, 2):
        ...
        if (t1_size > 0):
            while (True):
                period_time_start_main = time_for_driver_main + dateutil.relativedelta.relativedelta(
                    hours=random.randint(0, 3))
                time_for_driver_main = period_time_start_main
                period_time_end_main = period_time_start_main + dateutil.relativedelta.relativedelta(
                ...
                for drivers_count in range(0, random.randint(10, int(NODRIVERS / 3))):
                    ...
                    period_time_start = period_time_start_main + dateutil.relativedelta.relativedelta(
                        minutes=random.randint(0, 40))
                    time_for_driver = time_for_driver_main + dateutil.relativedelta.relativedelta(
                        minutes=random.randint(0, 40))
                    period_time_end = period_time_end_main + dateutil.relativedelta.relativedelta(
                        minutes=random.randint(0, 40))

                    while (time_for_driver < period_time_end):
                        start_time = time_for_driver + dateutil.relativedelta.relativedelta(
                            minutes=random.randint(1, 50)) + dateutil.relativedelta.relativedelta(
                        ...
                        to_insert = "insert into FAKTURY values (" + "\'" + CI_number + "\'" + ", " + "\'" + klient + "\'" + ", " + "\'" + kierowca + "\'" + ", " + "\'" + str(
                            trasa) + "\'" + ", " + "\'" + str(oplata) + "\'" + ", " + "\'" + str(
                            kilometry) + "\'" + ", " + "\'" + start_time.strftime(
                            "%Y-%m-%d %H:%M:%S") + "\'" + ", " + "\'" + end_time.strftime(
                            "%Y-%m-%d %H:%M:%S") + "\'" + ", " + "\'" + str(status_platnosci) + "\'" + ");\n"
